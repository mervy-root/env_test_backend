docker exec -it laravel_app bash -c 'cat > resources/views/tickets/edit.blade.php' << 'EOL'
@extends('layouts.app')
            ...
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form method="POST" action="{{ route('tickets.update', $ticket) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="event_name" id="event_name" value="{{ old('event_name', $ticket->event_name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                        @error('event_name')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="date" id="date" value="{{ old('date', $ticket->date->format('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                        @error('date')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="quantity" id="quantity" min="1" value="{{ old('quantity', $ticket->quantity) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                        @error('quantity')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
